...
- [ ] 3. 사용자가 지역/사업유형/절차단계/기준일로 질의를 필터링할 수 있다
      **부분 구현**: 지역(region)·사업유형(businessType)·기준일(referenceDate)은
      `hybridSearch`가 실제로 필터링하지만, **절차단계(procedureStage)는 `ChatSession`에
      저장만 되고 검색 필터로는 아직 반영되지 않는다.** Stage 1 감사 항목으로 등록,
      수정 필요.
...
- [ ] 17. 핵심 기능을 휴대전화에서 사용할 수 있다
      **Preview 배포까지는 완료**(PR #3), 그러나 **이번 병합 이후(실제 인증 포함)
      버전을 실제 휴대전화 화면에서 재확인하지는 않았다.** Stage 1 감사의 "모바일
      화면" 항목에서 재확인 예정.
...
- 완료: 14/18
- 부분 구현/재확인 필요: 2/18 (③ 절차단계 필터 미반영, ⑰ 모바일 실기기 재확인 필요)
